...
-- THIS IS THE HEAVILY MODIFIED FAILED APROACH COPY
...
-- resetn(pos): performs a hardware reset
--
-- The Sandbox
...
-- This controller doesn't support ports, only digilines to keep things very simple
-- The main benefit of this async_controller over luacontroller is obvious, it runs async
...
local function run_inner(pos, code, event, mem)
	-- Note: These return success, presumably to avoid changing LC ID.
	if overheat(pos) then return {
		ok=false,
		errmsg="overheated: source: mesecons",
		overheat=true,
		pos=pos,
	} end
	...
	if not env then return {
		ok=false,
		errmsg="Env does not exist. Controller has been moved?",
		overheat=false,
		pos=pos
	} end
	...
	if not f then return {ok=false, errmsg=msg, overheat=false, pos=pos, env=env} end
	...
	for _, v in ipairs(itbl) do
		local failure = v()
		if failure then
			return {ok=false, errmsg=failure, overheat=false, pos=pos,env=env}
		end
	end
	return {ok=true, errmsg=warning, overheat=false, pos=pos,env=env}
end
...
local function run_callback(ret)
	if not ret.ok then
		reset_meta(ret.pos, code, ret.errmsg)
	else
		reset_formspec(meta, code, ret.errmsg)
	end
	if should_burn then
		burn_controller(ret.pos)
	end
	if ret.env then
		save_memory(pos, meta, ret.env.mem)
	end
end
-- Wraps run_inner with LC-reset-on-error
-- also does the asyncing
local function run(pos, event)
	local meta = minetest.get_meta(pos)
	local code = meta:get_string("code")
	local mem = load_memory(meta)
	minetest.handle_async(run_inner, run_callback, pos, code, event, mem)
end
