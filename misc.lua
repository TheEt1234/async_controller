...
local function save_memory(pos, meta, mem)
	...
	if memstring == "Serializer took too long!" then
		...
		async_controller.env.reset_formspec(meta, meta:get_string("code"),
			"Error: Serializer took too long to execute!\nController overheats.")
		return
	end
	...
	if (#memstring <= memsize_max) then
		...
		async_controller.env.reset_formspec(meta, meta:get_string("code"),
			"Error: Luacontroller memory overflow. " .. memsize_max .. " bytes available, "
			.. #memstring .. " required.\nController overheats.")
		burn_controller(pos)
	end
end
